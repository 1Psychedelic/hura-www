...
const Event = (props) => {
    let createButton = () => {
        ...
        return (
            <Link to={props.event.url}>Zobrazit</Link>
        );
    };

    return (
        <div key={props.event.id} className="homepage-event">
            <Link to={props.event.url}><img className="homepage-event-banner" src={props.event.banner} alt={props.event.name}/></Link>
            <div className="homepage-event-content">
                <div>
                    <h2><Link to={props.event.url}>{props.event.name}</Link></h2>
