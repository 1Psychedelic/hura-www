...
class FacebookConnect extends React.Component {
        ...
        FB.login((response) => {
            if (response.status === 'connected' && response.authResponse && response.authResponse.signedRequest && response.authResponse.grantedScopes) {
                if (!response.authResponse.grantedScopes.includes('email')) {
                    ...
                    this.props.api.request('POST', 'api/v1/authentication/facebook-connect', data, (response, data) => {
                        if (response.status === 200) {
                            this.props.addAlert('login', 'success', 'Přihlašování přes Facebook je nyní aktivní.');
                            this.setState({
                                'isWorking': false,
                                'rerequestEmail': false
                            });
                ...
                this.props.addAlert('login', 'danger', 'Přihlašování přes Facebook se nepodařilo nastavit.');
                this.setState({'isWorking': false});
            }
        }, params);
    }
    ...
    render() {
        ...
        if (this.props.authentication.userProfile.loginMethods.facebook) {
            return (
                <div>
                    <div className="login-button login-button-facebook disabled"><img src="/images/icons/icon-facebook.png" alt="Facebook logo"/> <span>přes <strong>Facebook</strong> je aktivní</span></div>
                </div>
            );
        }
        ...
        return (
            <div>
                <a className="login-button login-button-facebook" onClick={this.onClick}><img src="/images/icons/icon-facebook.png" alt="Facebook logo"/> <span>Připojit <strong>Facebook</strong> účet</span></a>
            </div>
        );
    }
