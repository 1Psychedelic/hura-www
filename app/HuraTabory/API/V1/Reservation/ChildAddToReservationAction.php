...
class ChildAddToReservationAction implements RequestHandlerInterface
{
    /** @var TokenUser */
    private $tokenUser;
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        $child = $this->orm->children->get((int)$body['childId']);
        if ($child === null || !$child->parents->has($user)) {
            return new EmptyResponse(404, $headers->toArray());
        }
        ...
        if ($applicationChild === null) {
            $applicationChild = ApplicationChild::createFromUserChild($draft, $child);
